...
const Framework7PhoneGap = {
  ...
  create() {
    ...
    app.phonegap = {
      ...
      internalBrowser(url) {
        if (!this.exec(arguments.callee.name, url)) document.location = url;
      },
      ...
      statusbar: {
        ...
        backgroundColorByHexString: function (hexString) {
          if (hexString === false) hexString = "00000000";

          if (hexString.charAt(0) === "#") hexString = hexString.substr(1);

          if (hexString.length === 3) {
            const split = hexString.split("");
            hexString =
              split[0] + split[0] + split[1] + split[1] + split[2] + split[2];
          }
        ...
        hide(setInsets = true) {
          const insets =
            app.data["settings"] && app.data["settings"]["windowInsets"];
        ...
        show(setInsets = true) {
          ...
          if (app.device.android && setInsets && insets)
            app
              .$(":root")[0]
              .style.setProperty("--f7-safe-area-top", insets.top + "px");
      ...
      exec(fn, params, event, def) {
        function call() {
          if (app.device.android) {
            ...
            if (
              window["webkit"] &&
              typeof webkit.messageHandlers[fn] === "object"
            ) {
