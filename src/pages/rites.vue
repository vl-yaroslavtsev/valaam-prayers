    ...
    <form @submit="donate">
      ...
      <f7-list strong-ios dividers-ios outline-ios>
        <f7-list-input 
          ...
          min="1"
          required
          v-model:value="donation"></f7-list-input>
      ...
      <f7-block 
        strong 
        outline 
        :class="{'display-none': !error.common}">
        {{ error.common }}
      </f7-block>
      ...
      <f7-block strong-ios outline-ios class="grid grid-cols-2 grid-gap">
        <f7-button
          fill
          type="submit"
          :disabled="loading">Пожертвовать</f7-button>
      </f7-block>
    </form>
  </f7-page>
</template>
...
const email = ref(localStorage.getItem("donation-email") || "vl.yaroslavtsev@gmail.com");
const error = ref({});
...
const renderWidget = (payment, { onDone = () => {}, onError = () => {} } = {}) => {
